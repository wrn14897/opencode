    ...
    mcp: {
      toggle: async (directory: string, name: string) => {
        ...
        await toggleMcp({
          ...
          authenticate: async () => {
            // Cloud / web flow: ask the server for the authorize URL and open
            // it in a new tab so the user can complete consent in their own
            // browser. After the OAuth provider redirects back to our
            // /mcp/oauth/callback endpoint, opencode exchanges the code for
            // tokens and the MCP status flips to "connected" on the next
            // refetch. The native `mcp.auth.authenticate` path (which spawns a
            // system browser via `open`) doesn't work in our containerised
            // deployment, so we drive it explicitly here.
            const resp = await sdk.mcp.auth.start({ name })
